...
{
    // Check that there is referenced GameRegistry.ini and Level source directory
    if(argc <= 2)
    {
    ...
    {
        if(strstr(argv[1], "GameRegistry.ini") == NULL)
        {
    ...
    GameRegistry *p_registry = new GameRegistry(registryFile);
    ...
    while(1)
    {
        ...
        if(p_level == NULL)
        {
            switch(p_registry->BASHengine->GetLevel())
            {
                case 1:
